...
class LocationService {
  ...
  Future<Result<LocationData, LocationFailure>> getCurrentLocation() async {
    try {
      ...
      final locationData = await _geocodePosition(
        _FakePosition(latitude: position.latitude, longitude: position.longitude),
      );
      _logger.i('$_tag: Lấy vị trí thành công → ${locationData.formattedAddress}');
      return Success(locationData);
    } on LocationException catch (e, st) {
      _logger.e('$_tag: LocationException', e, st);
      return FailureResult(LocationFailure(e.message ?? 'Không thể lấy vị trí'));
    } catch (e, st) {
      _logger.e('$_tag: Unexpected error', e, st);
      return FailureResult(const LocationFailure('Lỗi không xác định khi lấy vị trí'));
    }
  }
  ...
  Future<Result<Position, LocationFailure>> getCurrentPosition() async {
    try {
      final position = await _determinePosition();
      _logger.i('$_tag: Toạ độ → lat=${position.latitude}, lng=${position.longitude}');
      return Success(position);
    } on LocationException catch (e, st) {
      _logger.e('$_tag: LocationException', e, st);
      return FailureResult(LocationFailure(e.message ?? 'Không thể lấy toạ độ'));
    } catch (e, st) {
      _logger.e('$_tag: Unexpected error', e, st);
      return FailureResult(const LocationFailure('Lỗi không xác định khi lấy toạ độ'));
    }
  }
  ...
  Future<Result<LocationData, LocationFailure>> getAddressFromCoordinates({
    ...
    try {
      ...
      _logger.i('$_tag: Geocode ($latitude, $longitude) → ${locationData.formattedAddress}');
      return Success(locationData);
    } on LocationException catch (e, st) {
      _logger.e('$_tag: LocationException', e, st);
      return FailureResult(LocationFailure(e.message ?? 'Không thể geocode toạ độ'));
    } catch (e, st) {
      _logger.e('$_tag: Unexpected error', e, st);
      return FailureResult(const LocationFailure('Lỗi không xác định khi geocode'));
    }
  }
